...
col_province = next((c for c in df.columns if "provinsi" in c.lower() or "prov" in c.lower()), df.columns[0])
col_commodity = next((c for c in df.columns if "komoditas" in c.lower() or "tanaman" in c.lower() or "jenis" in c.lower()), df.columns[1])
col_production = next((c for c in df.columns if "produksi" in c.lower() or "ton" in c.lower() or "produksi" in c.lower()), None)
col_area = next((c for c in df.columns if "luas" in c.lower() or "ha" in c.lower()), None)
col_productivity = next((c for c in df.columns if "produktivitas" in c.lower() or "rendemen" in c.lower()), None)

if col_production is None:
    # Fallback: kolom numerik pertama yang bukan tahun
    numeric_cols = df.select_dtypes(include=[np.number]).columns
    col_production = numeric_cols[0] if len(numeric_cols) > 0 else None
...
st.markdown(
    ...
    <div class="main-header">
        ...
        <p>Analisis Interaktif Data Perkebunan Indonesia — Tahun 2025 | UAS Data Science</p>
    </div>
    """,
    unsafe_allow_html=True,
)
